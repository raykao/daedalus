...
- [ ] **5.5 TTL cleanup** - `scripts/aks-cleanup.sh` listing RGs by tag, parsing `expires-at`, destroying expired groups. Scheduled GHA `nightly-cleanup.yml` runs daily at 09:00 UTC plus `workflow_dispatch`. Dry-run mode.
