...
`scripts/aks-cleanup.sh` lists resource groups tagged `auto-destroy=true`, parses `expires-at`, and destroys any whose timestamp is in the past. A scheduled GHA workflow (`nightly-cleanup.yml`, daily at 09:17 UTC) runs the script with appropriate workload identity. Manual invocation is supported for ad-hoc cleanup via `workflow_dispatch`.
